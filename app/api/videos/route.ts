...
export async function POST(request: NextRequest) {
  try {
    ...
    if (role === 'subject_admin' && assignedSubject && !subject) {
      throw new ApiError(400, `Subject is required — you are assigned to ${assignedSubject}`)
    }
    if (subject) assertSubjectMatch(role, assignedSubject, subject)
